...
var IMDBScraper = module.exports = function IMDBScraper() {
    ...
    this.namesPath = __dirname + '/names.json';
    this.CONFIG = {
        minWidth: 800,
        minHeight: 800,
        minValidImages: 20,
...
IMDBScraper.prototype.setNamesPath = function(path) {
    this.namesPath = path;
    this.names = require(path);
};
...
IMDBScraper.prototype.addName = function(name) {
    ...
    self = this;
    fs.writeFile(self.namesPath, JSON.stringify(this.names, null, 4), function(err) {
        if(err) {
          logger.log('error', err);
        } else {
          logger.log('debug', "JSON saved to " + self.namesPath);
        }
    });
};
